# IndiLingo Unified Merge Summary

## Executive summary

The 13 public repositories are best treated as one evolving product family, not as competing implementations. The final product should keep the recognizable umbrella name **IndiLingo** for the repository and public story, while using **IndiLingo** as the shorter app/product name inside UI. If a stronger final name is wanted later, use **BhashaPath** for a more enterprise-safe brand; for now, IndiLingo has the most continuity.

The recommended baseline is:

- **Primary repo shell:** `IndiLingo`
...
- **Best contributor/process docs:** `IndiLingo/docs/contributing`, `Indore03`, and `Indore04`
- **Best PWA/deployment direction:** primary `IndiLingo` plus Netlify/Vercel configs from later repos
...
- `IndiLingo/docs/contributing/*`
- `IndiLingo/PLANNING NOTES/*`
...
| API | Express/OpenAPI-first API with generated React Query client and Zod schemas | IndiLingo, Indore10 |
...
- `APPLY_TO_UPDATED_REPO.md` - commands tailored to the current updated local `IndiLingo` clone
